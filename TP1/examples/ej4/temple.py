...
def templado(productos,table):
    ...
    diccionario = {}
    agentee=agente.agente(table)

    for i in resultado:
        agentee.definir_agente(i[0],i[1],table)
        if agentee.objetivo==None:
            continue
        #g = funcion_a_estrella.A_estrella(inicio, fin)
        g=agentee.busqueda(table)  
    ...
    j=0
    for i in vector_p:
        if i==bahia_inicio_descarga:
            j+=1
            continue
        else:
            vector_p[j]=agentee.buscar_casilla(table,i[0])
            j+=1
